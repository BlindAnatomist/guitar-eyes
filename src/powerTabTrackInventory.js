...
const BASS_SUPPORTED_SOURCE_VERSIONS = new Set([
  "PT2_V11",
  "PTB_V10",
  "PTB_V102",
  "PTB_V15",
...
function inventoryItem(
  ...
  const bassSourceSupported = BASS_SUPPORTED_SOURCE_VERSIONS.has(sourceVersion);
  ...
  const standardBass =
    bassSourceSupported && standardBassTuning && coherentCoordinates;
  ...
  } else if (stringCount === 4 && !bassSourceSupported) {
    reasonCode = "UNSUPPORTED_PROFILE";
    reason =
      "Four-string bass is not accepted for this PowerTab source-version profile.";
  ...
  const instrument =
    standardBass ? "bass" : stringCount === 6 ? "guitar" : null;
