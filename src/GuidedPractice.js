...
import { describeLocation, describeWhatToPlay } from "./IPhoneTabReader";
...
const GuidedPractice = forwardRef(function GuidedPractice({ document }, headingRef) {
  ...
  useLayoutEffect(() => {
    if (pendingFocusRef.current === "instruction") {
      instructionHeadingRef.current?.focus({ preventScroll: true });
    }
    if (pendingFocusRef.current === "complete") {
  ...
  const completedCount = complete ? document.positions.length : currentIndex;
  ...
  return (
    <section className="guided-practice" aria-labelledby="guided-practice-heading">
      <h2 id="guided-practice-heading" ref={headingRef} tabIndex="-1">
        Guided practice
      </h2>
      ...
      {!started ? (
        <>
          <p>
            Practice the tablature one complete musical event at a time. After you play the
            instruction, choose Played it to receive the next one.
          </p>
        ...
        <section aria-labelledby="practice-complete-heading">
          <h3 id="practice-complete-heading" ref={completionHeadingRef} tabIndex="-1">
            Practice complete
          </h3>
          <p>You completed all {document.positions.length} instructions.</p>
          <button type="button" onClick={restartPractice}>
            Practice again from the beginning
          </button>
        </section>
      ) : (
        <>
          <section aria-labelledby="practice-instruction-heading">
            <h3 id="practice-instruction-heading" ref={instructionHeadingRef} tabIndex="-1">
              Practice instruction
            </h3>
            <p className="practice-progress">
              {completedCount} of {document.positions.length} instructions completed.
            </p>
          ...
          <fieldset className="practice-controls">
            ...
            <button type="button" onClick={moveBack} disabled={currentIndex === 0}>
              Back one instruction
            </button>
            <button type="button" onClick={confirmPlayed}>
              {currentIndex === document.positions.length - 1
                ? "Played it — finish practice"
                : "Played it — next instruction"}
            </button>
            <button type="button" onClick={() => announce(`${locationText} ${playingText}`)}>
              Repeat instruction
            </button>
            <button type="button" onClick={restartPractice} disabled={currentIndex === 0}>
