...
describe("checkpoint build identity", () => {
  test("places one format-only extended-string intake identity before the React root", () => {
    const html = fs.readFileSync(
      path.join(process.cwd(), "public", "index.html"),
      "utf8"
    );
    const title =
      "<title>Test build Guitar Eyes format-only extended-string ASCII intake proof 4B</title>";
    const heading =
      '<h1 id="test-build-heading">Test build: Guitar Eyes format-only extended-string ASCII intake proof 4B.</h1>';
