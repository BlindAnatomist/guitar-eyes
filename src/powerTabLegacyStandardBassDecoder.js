...
const VERSION_PROFILES = new Map([
  ...
  [4, { sourceVersion: "PTB_V17", powerTabVersion: "1.7", signature: "ptab-4" }],
]);

function fail(message, code = "INVALID_POWERTAB_STANDARD_BASS") {
...
class Reader {
  ...
  require(count, label = "data") {
    if (!Number.isInteger(count) || count < 0 || this.offset + count > this.bytes.length) {
      fail(
        `The PowerTab bass file ended unexpectedly while reading ${label}.`,
  ...
  count(label, maximum = MAX_VECTOR_ITEMS) {
    ...
    if (value > maximum) {
      fail(`${label} contains ${value} entries; the bounded limit is ${maximum}.`, "POWERTAB_LEGACY_COUNT_LIMIT");
    }
    return value;
  }
  string(label) {
...
function readEmptyVector(reader, label) {
  ...
  if (count !== 0) {
    fail(`${label} contains ${count} entries outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_SCORE_STRUCTURE");
  }
}
...
function parseModernTimeSignature(reader, context) {
  ...
  if (numerator !== 4 || denominator !== 4 || pulses !== 4) {
    fail(`${context} uses a time signature outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_TIME_SIGNATURE");
  }
...
function parseModernBarline(reader, context) {
  ...
  if (key !== 0) {
    fail(`${context} uses a key-signature change outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_KEY_SIGNATURE");
  }
  ...
  if (rehearsal !== 127 || rehearsalText !== "") {
    fail(`${context} uses rehearsal-sign data outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_REHEARSAL_SIGN");
  }
  const barType = data >>> 5;
  const repeatCount = data & 0x1f;
  if (barType !== 0 || repeatCount !== 0) {
    fail(`${context} uses repeat or non-simple barline data outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_BARLINE");
  }
...
function parseOldBarline(reader, context) {
  ...
  if (key !== 0 || data !== 0) {
    fail(`${context} uses key or repeat data outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_BARLINE");
  }
  return { position, numerator: 4, denominator: 4 };
}

function parseNote(reader, classes, context) {
  ...
  if (stringIndex >= STANDARD_BASS.length) {
    fail(`${context} references a string outside the four-string bass staff.`, "POWERTAB_LEGACY_STRING_OUT_OF_RANGE");
  }
  if (reader.u16(`${context} simple technique flags`) !== 0) {
    fail(`${context} uses note techniques outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_NOTE_TECHNIQUE");
  }
  ...
  for (let index = 0; index < complexCount; index += 1) {
    if (reader.u32(`${context} complex symbol ${index + 1}`) !== 0) {
      fail(`${context} uses a complex note symbol outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_NOTE_TECHNIQUE");
    }
  }
  return {
    stringNumberLowToHigh: STANDARD_BASS.length - stringIndex,
    fret,
    visible: true,
    isDead: false,
    techniques: [],
...
function parsePosition(reader, classes, context) {
  ...
  if (beaming !== 0) {
    fail(`${context} uses beaming data outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_BEAMING");
  }
  ...
  if ((flags & ~0x04) !== 0) {
    fail(`${context} uses position flags outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_POSITION_PROPERTY");
  }
  ...
  for (let index = 0; index < complexCount; index += 1) {
    if (reader.u32(`${context} complex symbol ${index + 1}`) !== 0) {
      fail(`${context} uses a complex position symbol outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_POSITION_PROPERTY");
    }
  }
  const noteCount = reader.count(`${context} notes`, STANDARD_BASS.length);
  const notes = [];
  for (let index = 0; index < noteCount; index += 1) {
    notes.push(parseNote(reader, classes, `${context}, note ${index + 1}`));
  }
  ...
  for (const note of notes) {
    if (usedStrings.has(note.stringNumberLowToHigh)) {
      fail(`${context} contains more than one note on the same string.`, "DUPLICATE_POWERTAB_LEGACY_STRING_AT_POSITION");
    }
...
function parseStaff(reader, classes, context) {
  classes.read(reader, "CStaff", context);
  const data = reader.u8(`${context} staff data`);
  if (data !== 0x14) {
    fail(`${context} is not the bounded four-string bass-clef tablature staff.`, "UNSUPPORTED_POWERTAB_LEGACY_STAFF");
  }
  ...
  for (let voiceIndex = 0; voiceIndex < 2; voiceIndex += 1) {
    ...
    for (let index = 0; index < count; index += 1) {
      positions.push(parsePosition(reader, classes, `${context}, voice ${voiceIndex + 1}, position ${index + 1}`));
    }
    voices.push({ index: voiceIndex, positions });
  }
  if (voices[1].positions.length !== 0) {
    fail(`${context} contains a second active voice outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_MULTIVOICE");
  }
  const coordinates = voices[0].positions.map((position) => position.sourcePosition);
  for (let index = 1; index < coordinates.length; index += 1) {
    if (coordinates[index] <= coordinates[index - 1]) {
      fail(`${context} positions are not in strictly increasing source order.`, "AMBIGUOUS_POWERTAB_LEGACY_POSITION_ORDER");
    }
  }
  return voices[0];
}

function parseBassPlayer(reader, classes, context) {
  classes.read(reader, "CGuitar", context);
  const number = reader.u8(`${context} number`);
  if (number !== 0) {
    fail(`${context} uses unexpected player number ${number}.`, "UNSUPPORTED_POWERTAB_LEGACY_PLAYER");
  }
  const description = reader.string(`${context} description`) || "PowerTab Bass";
  ...
  if (capo !== 0) {
    fail(`${context} uses capo ${capo} outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_CAPO");
  }
  ...
  if (
    stringCount !== STANDARD_BASS.length ||
    tuning.some((value, index) => value !== STANDARD_BASS[index])
  ) {
    fail(`${context} uses tuning outside exact standard G2 D2 A1 E1.`, "UNSUPPORTED_POWERTAB_LEGACY_TUNING");
  }
...
function splitBars(positions, starts) {
  for (let index = 1; index < starts.length; index += 1) {
    if (starts[index] <= starts[index - 1]) {
      fail("PowerTab bass barlines are not in strictly increasing source order.", "AMBIGUOUS_POWERTAB_LEGACY_BARLINE_ORDER");
    }
  }
  let absoluteTick = 0;
  return starts.map((start, index) => {
    ...
    if (selected.length === 0) {
      fail(`Measure ${index + 1} contains no positions in the standard four-string bass profile.`, "EMPTY_POWERTAB_LEGACY_MEASURE");
    }
    let relativeTick = 0;
    ...
    return {
      ...
      repeatCount: 0,
      alternateEndings: 0,
      voices: [{ index: 0, beats }, { index: 1, beats: [] }],
    };
  });
}

function parseSystem(reader, classes, context, fileVersion) {
  ...
  if (fileVersion <= 2) {
    ...
    if (key !== 0 || ((endBar >>> 8) & 0xff) !== 1 || (endBar & 0xff) !== 0) {
      fail(`${context} uses old-system key or final-bar data outside the standard four-string bass profile.`, "UNSUPPORTED_POWERTAB_LEGACY_BARLINE");
    }
  ...
  if (staffCount !== 1) {
    fail(`${context} contains ${staffCount} staves; the standard four-string bass profile requires one.`, "UNSUPPORTED_POWERTAB_LEGACY_STAFF_COUNT");
  }
  const voice = parseStaff(reader, classes, `${context}, staff 1`);
  ...
  for (let index = 0; index < barlineCount; index += 1) {
    ...
    const barline = fileVersion <= 2
      ? parseOldBarline(reader, `${context}, internal barline ${index + 1}`)
      : parseModernBarline(reader, `${context}, internal barline ${index + 1}`);
    internalStarts.push(barline.position);
  }
  return splitBars(voice.positions, [firstStart, ...internalStarts]);
}

function parseEmptyScore(reader, context) {
  for (const label of [
    "players",
    "chord diagrams",
    "floating text",
...
function parseBassScore(reader, classes, context, fileVersion) {
  const playerCount = reader.count(`${context} players`, 32);
  if (playerCount !== 1) {
    fail(`${context} contains ${playerCount} players; the standard four-string bass profile requires exactly one.`, "UNSUPPORTED_POWERTAB_LEGACY_BASS_PLAYER_COUNT");
  }
  const bass = parseBassPlayer(reader, classes, `${context}, bass 1`);
  for (const label of [
    "chord diagrams",
    "floating text",
    "guitar-in",
  ...
  if (systemCount !== 1) {
    fail(`${context} contains ${systemCount} systems; the standard four-string bass profile requires exactly one.`, "UNSUPPORTED_POWERTAB_LEGACY_SYSTEM_COUNT");
  }
  const bars = [];
  for (let index = 0; index < systemCount; index += 1) {
    bars.push(...parseSystem(reader, classes, `${context}, system ${index + 1}`, fileVersion));
  }
  return {
    ...bass,
    staves: [{ tuningMidiHighToLow: bass.tuningMidiHighToLow, bars }],
  };
}
...
function parseHeader(reader) {
  ...
  if (!profile) {
    fail(`The standard bass decoder accepts PowerTab file-version values 1 through 4; this file reports ${fileVersion}.`, "INVALID_POWERTAB_LEGACY_VERSION");
  }

  if (fileVersion === 4) {
    const fileType = reader.u8("PowerTab file type");
    if (fileType !== 0) {
      fail("The standard four-string bass profile accepts song files only.", "UNSUPPORTED_POWERTAB_LEGACY_FILE_TYPE");
    }
    const contentType = reader.u8("PowerTab content type");
    if (contentType !== 0x02) {
      fail("The standard four-string bass profile requires bass-only PowerTab 1.7 content.", "UNSUPPORTED_POWERTAB_LEGACY_CONTENT_TYPE");
    }
    ...
    if (releaseType === 0) {
      ...
      fail(`The legacy PowerTab header contains invalid release type ${releaseType}.`, "INVALID_POWERTAB_LEGACY_HEADER");
    }
    ...
    if (authorType === 0) {
      ...
      fail(`The legacy PowerTab header contains invalid author type ${authorType}.`, "INVALID_POWERTAB_LEGACY_HEADER");
    }
    ...
    return { fileVersion, title: title || "PowerTab 1.7 bass tablature", artist, ...profile };
  }
  ...
  return { fileVersion, title: title || `PowerTab ${profile.powerTabVersion} bass tablature`, artist, ...profile };
}
...
export function decodePowerTabLegacyStandardBassBytes(
  input,
  ...
  if (bytes.length === 0 || bytes.length > maxFileBytes) {
    fail(`The selected legacy PowerTab bass file must be between 1 and ${maxFileBytes} bytes.`, "POWERTAB_LEGACY_FILE_SIZE_LIMIT");
  }
  const reader = new Reader(bytes);
  const classes = new ClassMap();
  const header = parseHeader(reader);
  parseEmptyScore(reader, "Guitar score");
  const bass = parseBassScore(reader, classes, "Bass score", header.fileVersion);
  ...
  if (lineSpacing < 1 || lineSpacing > 100) {
    fail("The legacy PowerTab document contains invalid line spacing.", "INVALID_POWERTAB_LEGACY_FOOTER");
  }
  if (reader.offset !== bytes.length) {
    fail(`The legacy PowerTab bass document contains ${bytes.length - reader.offset} trailing bytes outside the bounded profile.`, "UNSUPPORTED_POWERTAB_LEGACY_TRAILING_DATA");
  }
  const tracks = [bass];
  const measureCount = bass.staves[0].bars.length;
  const historical = header.fileVersion <= 3;
  return {
    schemaVersion: 1,
    ...
    versionEvidence: {
      ...
      ...(historical
        ...
        : { independentSignature: header.signature }),
...
export async function decodePowerTabLegacyStandardBassFile(file, options = {}) {
  if (!file || typeof file.arrayBuffer !== "function") {
    fail("A readable legacy PowerTab .ptb bass file is required.", "INVALID_POWERTAB_LEGACY_FILE");
  }
  if (!/\.ptb$/iu.test(String(file.name || ""))) {
    fail("The legacy PowerTab standard bass decoder accepts .ptb files only.", "INVALID_POWERTAB_LEGACY_EXTENSION");
  }
  const maxFileBytes = options.maxFileBytes || 5 * 1024 * 1024;
  if (!Number.isInteger(file.size) || file.size <= 0 || file.size > maxFileBytes) {
    fail(`The selected legacy PowerTab bass file must be between 1 and ${maxFileBytes} bytes.`, "POWERTAB_LEGACY_FILE_SIZE_LIMIT");
  }
  return decodePowerTabLegacyStandardBassBytes(new Uint8Array(await file.arrayBuffer()), options);
}
